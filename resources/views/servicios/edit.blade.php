...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('servicios.update', $servicio) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('put')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Editar servicio') }}</h4>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('categoria_id') ? ' has-danger' : '' }}">  
                      <select class="form-control{{ $errors->has('categoria_id') ? ' is-invalid' : '' }}" id="input-categoria_id" required="true" aria-required="true" name="categoria_id"> 
                        <option value="{{ old('categoria_id', $servicio->categoria->id) }}">{{ $servicio->categoria->nombre}}</option>
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('prioridad_id') ? ' has-danger' : '' }}">  
                      <select class="form-control{{ $errors->has('prioridad_id') ? ' is-invalid' : '' }}" id="input-prioridad_id" required="true" aria-required="true" name="prioridad_id"> 
                        <option value="{{ old('prioridad_id', $servicio->prioridad->id) }}">{{ $servicio->prioridad->prioridad}}</option>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('nombre') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('nombre') ? ' is-invalid' : '' }}" name="nombre" id="input-nombre" type="text" placeholder="{{ __('Nombre') }}" value="{{ old('nombre', $servicio->nombre) }}" required="true" aria-required="true"/>
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('sigla') ? ' has-danger' : '' }}">
                      <input class="form-control{{ $errors->has('sigla') ? ' is-invalid' : '' }}" name="sigla" id="input-sigla" type="text" placeholder="{{ __('sigla') }}" value="{{ old('sigla', $servicio->sigla) }}" required="true" aria-required="true"/>
                      @if ($errors->has('sigla'))
                  ...
                  <div class="col-sm-7">
                    <div class="form-group{{ $errors->has('estado_id') ? ' has-danger' : '' }}">
                      <div class="togglebutton">
                        <label>
                          <input name="estado_id" type="checkbox" {{ $servicio->estado_id == 1 ? ' checked' : '' }}  value="{{ old('estado_id', 1) }}">
